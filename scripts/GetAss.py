...
from crawler.scripts.support import *

import sys, zlib, os, math, random, xml.dom.minidom, io, hashlib
...
def GetVideoInfo(aid,appkey,page = 1,AppSecret=None,fav = None):
    ...
    return video
def GetTitle(aid,appkey,page = 1,AppSecret=None,fav = None):
    paras = {'id': GetString(aid),'page': GetString(page)}
    if fav != None:
        paras['fav'] = fav
    url =  'http://api.bilibili.cn/view?'+GetSign(paras,appkey,AppSecret)
    jsoninfo = JsonInfo(str(url))
    video = Video(aid,jsoninfo.Getvalue('title'))
    
    return video.title

def getViews(aid,appkey,page = 1,AppSecret=None,fav = None):
    paras = {'id': GetString(aid),'page': GetString(page)}
    if fav != None:
        paras['fav'] = fav
    url =  'http://api.bilibili.cn/view?'+GetSign(paras,appkey,AppSecret)
...
def GetDanmuku(cid):
    ...
    content = zlib.decompressobj(-zlib.MAX_WBITS).decompress(getURLContent_byte(url))
...
def ReadCommentsBilibili(f, fontsize):
    ...
    for i, comment in enumerate(comment_element):
        try:
            ...
            assert len(p) >= 5
            publish_time = int(p[4])
            assert p[1] in ('1', '4', '5', '6', '7')
            if p[1] != '7':
                c = str(comment.childNodes[0].wholeText).replace('/n', '\n')
                size = int(p[2])*fontsize/25.0
                yield (float(p[0]), publish_time , i, c, {'1': 0, '4': 2, '5': 1, '6': 3}[p[1]], int(p[3]), size, (c.count('\n')+1)*size, CalculateLength(c)*size)
            else:  # positioned comment
                c = str(comment.childNodes[0].wholeText)
                yield (float(p[0]), publish_time, i, c, 'bilipos', int(p[3]), int(p[2]), 0, 0)
        except (AssertionError, AttributeError, IndexError, TypeError, ValueError):
            continue

def ConvertToFile(filename_or_file):
    return open(filename_or_file, 'w',encoding='utf-8')
...
def Danmaku2ASS(input_files, output_file, stage_width, stage_height, reserve_blank=0, font_face='sans-serif', font_size=25.0, text_opacity=1.0, comment_duration=5.0, is_reduce_comments=False, progress_callback=None):
    status = 0
    # status = 1 means successfully download
    fo = None
    if not input_files:
        print("Link not valid anymore")
        return -1
    comments = ReadComments(input_files, font_size)
    if len(comments) < 100:
        print("not enough danmu in vid {}".format(output_file))
        return status
...
def download_danmu(vid, path='../danmus'):
    url = "https://www.bilibili.com/video/{}/".format(vid)
    # print("Downloading danmu from {}".format(vid))
    ...
    status = Danmaku2ASS(GetDanmuku(video.cid), os.path.join(path,"{}.ass".format(vid)), 640, 360, 0, 'sans-serif', 15, 0.5, 10, False)
    return status

def download_title(vid):
    url = "https://www.bilibili.com/video/{}/".format(vid)
    appkey = "03fc8eb101b091fb"
    regex = re.compile('https:/*[^/]+/video/av(\\d+)(/|/index.html|/index_(\\d+).html)?(\\?|#|$)')
    regex_match = regex.match(url)
    ...
    aid = regex_match.group(1)
    pid = regex_match.group(3) or '1'
    title = GetTitle(aid, appkey, AppSecret=None, page=pid)
    # print(title)
    return title
def get_views(vid):
    url = "https://www.bilibili.com/video/{}/".format(vid)
    ...
    aid = regex_match.group(1)
    pid = regex_match.group(3) or '1'
    views = getViews(aid, appkey, AppSecret=None, page=pid)
    # print(title)
    return views
if __name__ == '__main__':
    # get_views("av41501461")
    download_danmu("av41501461")
    # sys.argv = ["", "https://www.bilibili.com/video/av25961598/"]
    # if len(sys.argv) != 2:
    #     print("输入av号")
    # else:
    #     appkey = "03fc8eb101b091fb"
    ...
    #     video = GetVideoInfo(aid, appkey, AppSecret=None, page=pid)
    #     # Danmaku2ASS(GetDanmuku(video.cid),r'%s/Desktop/%s-%s.ass'%(os.path.expanduser('~'),video.title,pid), 640, 360, 0, 'sans-serif', 15, 0.5, 10, False)
    #     Danmaku2ASS(GetDanmuku(video.cid), "danmu.ass", 640, 360, 0, 'sans-serif', 15, 0.5, 10, False)